...
# โครงสร้างสำหรับการเก็บข้อมูลใน Session (เน้นตัวข้อมูลหลักก่อน)
...
# ฟังก์ชันจัดหน้าตาราง Excel ให้เต็มฟอร์ม 12 คอลัมน์ตามตัวอย่างของพี่
def build_full_excel(data_list):
    ...
    for row in data_list:
        ...
        # ตั้งค่าตัวแปรอัตโนมัติให้สอดคล้องกับ สน. นครบาล (พท.ตอนใน)
        ...
        full_rows.append({
            'สน.': row['สน.'],
            'ผู้ต้องหา\n( คน )': count if count > 0 else 0,
            'สัญชาติ': row['สัญชาติ'],
            'ผู้ต้องหา พท.ตอนใน': pht_in,
            'ผู้ต้องหา พท.ติดชายแดน': 0,
            'สถานที่ที่จับกุม': row['สถานที่ที่จับกุม'],
            'จังหวัด': prov,
            'การลักลอบ พื้นที่ช่องทาง': "-",
            'การลักลอบ เข้ามาเอง': "-",
            'การตรวจ COVID-19': "-",
    ...
    # เพิ่มแถวสรุปผลรวมที่ท้ายตาราง
    full_rows.append({
        'สน.': 'รวม', 'ผู้ต้องหา\n( คน )': total_count, 'สัญชาติ': '', 'ผู้ต้องหา พท.ตอนใน': total_count,
        'ผู้ต้องหา พท.ติดชายแดน': 0, 'สถานที่ที่จับกุม': '', 'จังหวัด': '', 'การลักลอบ พื้นที่ช่องทาง': '',
        'การลักลอบ เข้ามาเอง': '', 'การตรวจ COVID-19': '', 'เดินทางมาก่อน 1963-10-01': '', 'หมายเหตุ': ''
    })
...
with col_right:
    ...
    # 📥 ส่งออก Excel ตัวเต็มคอลัมน์ครบถ้วนตามแบบฟอร์มรายงานของจริง
    excel_data = build_full_excel(st.session_state.report_data)
    st.download_button(
        label="📥 ดาวน์โหลดไฟล์ Excel สำหรับส่งรายงาน (.xlsx)",
        data=excel_data,
        file_name="แบบรายงานการจับกุมต่างด้าว_ฟอร์มเต็ม.xlsx",
